...
Connect an ESP-01 with adaptor board to ATMEGA162 program port.

On PC side use ExtraPuTTY or Tera Term to transfer via Telnet.
On TI side use Xmodem program on this disk.

(The programs below are outdated, Instead use CALL VOLMGR F5 to send & F6 to receive)
...
Hookup for programming
 ...
 RX<--TX| (TX )(GND) GND/        |         | (CTS)                      |
        --------------------------      GND| (GND)                      |
                                           ------------------------------
[Use ESP-01 adaptor board to ATMEGA162]
...
I use a large cap 470uF as the peak only comes up periodically.
