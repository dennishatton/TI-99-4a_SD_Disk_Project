...
FindTIVOL:
	...
	ldi ZL, low(SDbuffer+0x1C6)	;Save VBR address
...
;;;;;;;	
;Add work around for larger than 8gb cards	
	ldi ZL, low(SDbuffer+0x5C)	;Get total clusters
	ldi ZH, high(SDbuffer+0x5C)	; "
	ldi YL, low(arg)    		; "
	ldi YH, high(arg)   		; "
	rcall mov4
	ldi r19, 14			;Check if greater than 8gb
	rcall LsrArg
	lds r16, arg
	cpi r16, 0
	breq JP08
	ldi r16, 8			; Put 2048 in arg
	sts arg+1, r16			; "
	clr r16				; "
	sts arg, r16			; "
	sts arg+2, r16			; "
	sts arg+3, r16			; '
	ldi YL, low(arg)		;Add 2048 to VBR offset
	ldi YH, high(arg)		; "
	ldi XL, low(vbr)    		; "
	ldi XH, high(vbr)   		; "
	rcall AddDword			; Result in VBR
;;;;;;;	
JP08:
	...
	ldi ZL, low(SDbuffer+0x6D)	;Point to SectorsPerCluster
	ldi ZH, high(SDbuffer+0x6D)	;
	ld r19, Z			;
	sts SectorsPerCluster, r19 	;Save SectorsPerCluster
	rcall LslArg			;RootDirectory * SectorsPerCluster

;************************************************************************************
; Process Root Directory Sector	(Find VOLUMES folder)
;************************************************************************************
	ldi XL, low(arg)		;Add VBR offset to RootDirectory in arg
	ldi XH, high(arg)		; "
	ldi YL, low(vbr)    		; "
	ldi YH, high(vbr)   		; "
	rcall AddDword			; "
...
;************************************************************************************
	ldi XL, low(arg)		;Add VBR offset to VOLUMES folder in arg
	ldi XH, high(arg)		; "
	ldi YL, low(vbr)    		; "
	ldi YH, high(vbr)   		; "
	rcall AddDword			; "
...
JP05:	sbiw ZH:ZL, 32			;Back up from type C1 entry to C0 entry
	...
	ldi XL, low(arg)		;Add VBR offset to TIVOL000 file in arg
	ldi XH, high(arg)		; "
	ldi YL, low(vbr)    		; "
	ldi YH, high(vbr)   		; "
	rcall AddDword			;
