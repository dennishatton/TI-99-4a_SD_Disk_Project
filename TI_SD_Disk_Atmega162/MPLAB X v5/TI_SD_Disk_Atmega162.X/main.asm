...
SetupDSK0:
	ldi r16,255		;DSK0 use default volume 255
	sts DSK0volume,r16
SetupDSK:
	ldi r16,0x01
...
;XL & XH ok to use before interrupts are enabled    	
	ldi ZL,low(SDbuffer+1)	    ;+1 skip DSK0
	ldi ZH,high(SDbuffer+1)	    ;

	lds r16,DSK0volume	    ;Get Drive 0 Disk #
	push r16		    ;DSK0 not saved in sector 1
	...
	ldi r18,4
lp02:	pop r21			    ;Restore disk number (stepping 3 - 0)
...
;Set last track for drives
	ldi ZL,low(DSK0address)    ;LSB -- MSB
	ldi ZH,high(DSK0address)   ;
	ldi YL,low(arg)	    ;
	ldi YH,high(arg)	    ;
	rcall mov4		    ;
	rcall ReadMMC 		    ;First half 512 sector (uses ZL & ZH)
...
Track2Arg:
	...
	rjmp jp10
	lds r17,DSK0LastTrk	;Drive 0 If below drives are skipped
	sbic PinD,4		;Skip If not drive 1
	lds r17,DSK1LastTrk	;
	sbic PinD,5		;Skip If not drive 2
	lds r17,DSK2LastTrk	;
	sbic PinD,6		;Skip If not drive 3
	lds r17,DSK3LastTrk	;
...
jp10:	
	...
	ldi ZH,high(DSK0address)    ;Start DSK0address
	ldi ZL,low(DSK0address)	    ; "
	sbic PinD,4		    ;Skip If not drive 1
	adiw Z,4		    ;DSK1address
	sbic PinD,5		    ;Skip If not drive 2
	adiw Z,8		    ;DSK2address
	sbic PinD,6		    ;Skip If not drive 3
	adiw Z,12		    ;DSK3address
...
;Send disk numbers DSK1-3 
	...
	ldi ZH,high(SDbuffer+1)	; Point to disk #s in buffer
	ldi ZL,low(SDbuffer+1)	; Skip DSK0 start with DSK1
...
;Get disk number from TI
	...
	cpi r20,0		;Check If DSK0
	brne jp23
	sts DSK0volume,r21	;Store DSK0 volume
	rjmp jp24		;But don't save in sector 1
...
; Drive number r20
; Disk number r21
	add ZL,r20		;Point to DSK0,DSK1,DSK2,DSK3	
	clr r16			;Note 1st byte DSK0 not used
	adc ZH,r16		; "
	st Z,r21		; Save new disk number
